...
\- \*\*最小差分:\*\* 依頼範囲外の変更（ついでリファクタ、不要な整形、無関係な命名変更）をしない。

\- \*\*既存方式を優先:\*\* 画面遷移・状態管理は\*\*既存のAutoload（GameManager／遷移エフェクト）\*\*に従う。直接 `change\_scene\_\*` を乱用しない。

\- \*\*必ず動作確認＋テスト:\*\* 変更後は最低限の手動確認と、必要に応じたGUTテスト追加/更新を行う。

\- \*\*コメントは「なぜ」中心:\*\* 追加/修正した箇所は日本語で簡潔に「意図・理由」を残す。
...
&nbsp; `res://root/scenes/boot\_splash\_scene/boot\_splash\_scene.tscn`

\- ゲーム選択（メインメニュー）シーン  

&nbsp; `res://root/scenes/main\_menu\_scene/main-menu-scene.tscn`

\- ミニゲーム配置ディレクトリ  

&nbsp; `res://root/scenes/game/`
...
&nbsp; 4) `scene\_changed` 等で切替完了を待つ  

&nbsp; 5) フェードイン開始



> ミニゲーム側で `get\_tree().change\_scene\_\*` を直接呼ぶ必要がある場合でも、まず GameManager に遷移APIを追加して統一する。
...
&nbsp; - `fade\_out()` / `fade\_in()` と完了シグナルを提供している想定。既存APIに合わせること。



> Autoloadを増やす場合は「責務」を最小にし、AGENT.mdにも追記する。
...
\- ミニゲームはそれぞれ独自シーンとして `res://root/scenes/game/` 配下に作成する。



推奨構成（例）:

\- `res://root/scenes/game/<game\_id>/<game\_id>.tscn`

\- `res://root/scenes/game/<game\_id>/<game\_id>.gd`
...
&nbsp; 例: `GameManager.start\_game(scene\_path\_or\_packed\_scene)`
...
&nbsp; 例: `GameManager.load\_main\_scene()`
...
\- クラス名（`class\_name`）: `PascalCase`

\- ファイル名: 原則 `snake\_case.gd`（既存に合わせる）
...
\- 追加/修正箇所には日本語で簡潔にコメント。

\- 「何をしているか」より \*\*“なぜこの実装か”\*\* を優先。
...
&nbsp; - 例: `godot -d -s --path "$PWD" addons/gut/gut\_cmdln.gd -gdir=res://test/unit -ginclude\_subdirs -gexit`
