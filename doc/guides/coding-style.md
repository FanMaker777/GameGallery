...
- **ノードの動的生成は最小限にする:** 構造が固定の UI（セクションヘッダー、ラベル、区切り線など）は `.tscn` に静的配置し、スクリプトは値の更新のみに責務を絞る。動的生成はデータ依存で要素数が変動する場合（例: 敵種別ごとの統計行）に限定する。値を更新する Label には `unique_name_in_owner = true` を設定し、`%Name` で参照する
