...
- **既存方式を優先:** 画面遷移・状態管理は既存のAutoload（GameManager／遷移エフェクト）に従う。
...
- 複雑な処理や意図が読みにくいコードには、 **なぜこの実装か**を追加でコメントする。
...
- プロジェクトにロガー（例: `Log`）がある場合はそれを優先し、`print()`は使用しない。
...
### 5.5 コード品質
***このプロジェクトでは常に「拡張性・保守性・可読性」を最優先して実装/改修すること。***

- 拡張性: 新しい設定/機能を追加する際に、既存コードの変更範囲が最小で済む構造にする（コピペ増殖を避ける）。
- 保守性: 表示文言やUI構造変更に強い実装にする（"文字列match" 依存を避ける）。責務を分離し、テスト可能な単位を作る。
- 可読性: 意図がコードから読み取れる命名・関数分割・データ駆動（辞書/定数/enum）を優先する。

#### 責務分離（UI / 適用 / 保存）
同一スクリプトにUI操作・設定適用（DisplayServer 等）・保存（ConfigFile 等）が混在する場合、
次の分離を検討する（小さく始めて良い）:

- View（UI）: OptionButton/CheckBox のイベント処理、表示更新のみ
- Applier（適用）: DisplayServer など副作用のある処理を集約
- Store（保存/読込）: 設定の永続化とデフォルト復元

「UIが内部値を渡すだけ」「Applierは副作用だけ」「StoreはI/Oだけ」を目標にする。

#### 内部クラス（inner class）を使う判断基準
内部クラスは「そのスクリプトに閉じた実装詳細」を整理するために使う。
使いどころ:
- そのファイル専用の小型DTO（引数/結果の束）
- 状態マシンの State 群
- View/Applier/Store を同一ファイル内で軽く分けたい（将来分割前の段階的整理）

注意点（必須）:
- 内部クラスが外側Nodeを強参照し続けない設計を優先する（必要なら依存を明示的に渡す）。
- RefCounted 同士の循環参照になり得る設計は避ける（必要なら WeakRef を検討）。
- Inspector で割り当てたい/Resourceとして保存したい型は内部クラスにしない（別スクリプト/Resource化を検討）。

#### スクリプト分割（別ファイル化）を使う判断基準
次に当てはまる場合は、内部クラスではなく「別スクリプトへ分割」を優先する。

- 複数シーン/複数スクリプトから再利用したい
- Inspectorで割り当てたい、または .tres/.res として保存したい（Resource化含む）
- 単体テスト対象として独立させたい（外部依存が少ない）
- ファイルが肥大化し、責務が3つ以上に見える（UI/保存/適用/検証/変換などが混在）

分割の指針:
- まず「適用（Applier）」や「変換/マッピング（Mapper）」など副作用が少ない領域から切り出す。
- 切り出したクラスは命名で責務が分かるようにする（例: `VideoSettingApplier`, `VideoSettingStore`）。
