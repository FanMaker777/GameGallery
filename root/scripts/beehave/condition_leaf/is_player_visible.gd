...
func tick(actor: Node, blackboard: Blackboard) -> int:
	...
	var is_visible: Variant = blackboard.get_value(BlackBoardValue.IS_PLAYER_VISIBLE)
	# プレイヤーが探知範囲内に存在しない場合
	if not is_visible:
		return FAILURE
	...
	blackboard.set_value(BlackBoardValue.PLAYER_POSITION, player.global_position)
	return SUCCESS
