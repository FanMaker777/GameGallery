...
## 攻撃クールダウン・攻撃範囲・攻撃ダメージは Blackboard 経由で EnemyData から取得する
...
func tick(actor: Node, blackboard: Blackboard) -> int:
	...
	# Blackboard から攻撃クールダウンを取得する
	var attack_cooldown: float = blackboard.get_value(BlackBoardValue.ATTACK_COOLDOWN)
...
func _apply_damage_to_player(actor: Node, blackboard: Blackboard) -> void:
	...
	if distance > attack_range:
		return
	# Blackboard から攻撃ダメージを取得してプレイヤーにダメージを与える
	var damage: int = blackboard.get_value(BlackBoardValue.ATTACK_DAMAGE)
